...
class GraphMaking:
    ...
    def __init__(self):
        ...
        self.order = 0
        self.infinities = []
        self.filee = 'karate_club_graph'

    def create_graph(self):
        self.graph = Graph()
        input_file = open(os.path.join('input',self.filee+'.txt'), 'r')
    ...
    def isolation(self):
        for i in range(0,self.order):
            ...
            for c in components_sizes:
                total = total - c
                count+= 2*c*total
            self.infinities.append(count)

    def create_results(self):
        output_local = open(os.path.join('output','output_'+self.filee+'_1.txt'), 'a')
